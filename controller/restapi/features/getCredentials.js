    ...
    if (process.env.VCAP_SERVICES) {
        ...
        for (let service_name in services) {
            if (service_name.indexOf(name) === 0) {
                let service = services[service_name][0];
                var creds = {};
...
exports.getServiceCreds = function(req, res, next)
{
    ...
    {
        let addConfig = {};
        let serviceList = require('../../env_2.json');
        ...
        {console.log('each is: '+each+' with value: '+addConfig[each]);
          ...
          {for (param in _config[_each])
            {(function(_param, __each, __config)
              {let newCreds = _config[__each][_param];
    ...
    else{let _path = path.join(path.dirname(require.main.filename),'controller','env.json');
