...
function printHelp ()
{
    ...
    echo -e "${GREEN}-n ${RESET}defaults to ${GREEN}eshop-network${RESET}. use ${YELLOW}-n eshop-network ${RESET}if you are using a different network name"  | indent
...
composer network install --card PeerAdmin@hlfv1 --archiveFile eshop-network.bna
...
# corrected to: 
# composer network start -c PeerAdmin@hlfv1 -A admin -S adminpw -a $NETWORK_NAME.bna --file eshop-network.card
composer network start --networkName eshop-network --networkVersion 0.1.5 --networkAdmin admin --networkAdminEnrollSecret adminpw --card PeerAdmin@hlfv1 --file eshop-network.card
# composer network start -a $NETWORK_NAME.bna -c PeerAdmin@hlfv1 -A admin -S adminpw --file eshop-network.card
showStep "importing eshop-network card"
if composer card list -c admin@$NETWORK_NAME > /dev/null; then
    composer card delete -c admin@$NETWORK_NAME
fi
composer card import --file eshop-network.card
